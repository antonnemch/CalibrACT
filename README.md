...
CalibrACT is the code artifact for my research internship paper, [*Domain Adaptation of Pretrained Convolutional Neural
Networks via Nonlinearity Calibration*](docs/report.pdf). The project studies whether a frozen ImageNet-pretrained ResNet-50 can be adapted to a low-resource medical imaging task by tuning only a tiny set of nonlinear activation parameters plus the classifier head.
